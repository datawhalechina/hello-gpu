...
我们先将一次优化拆成三个部分：模型选择行动，工具执行评测，工作区保存结果。
...
在开始搜索之前，我们需要把算子源码和输入条件整理成工具能够读取的任务规格。
...
预置任务初始化时会把基线复制为 `best.py`，同时保留原始 `baseline.py`。这次复制只建立起点。第 17 章的运行入口随后执行正确性预检和基线计时，分别保存 `preflight.json` 与 `baseline-evaluation.json`，通过后才进入搜索。如果直接调用更底层的 Agent 入口，仍应自行确认基线已经检查和测量。

## 16.3 让一项假设对应一份候选

为了看清一次修改的作用，我们让每份候选只检验一个主要假设。
...
候选已经测得了时间，却还没有决定是否保留；程序需要记住这个中间状态。
...
完成一轮评测后，我们将结果放回对话，作为模型选择下一步行动的依据。
...
连续尝试需要一个停止条件。我们先给模型设置调用预算，再检查预算耗尽时是否还有未处理的候选。
...
第 17 章的运行入口会检查结束状态，并在搜索后执行独立复测。程序正常退出后，我们还需要结合 `trajectory.jsonl` 和最终源码分析结果，判断哪些修改被保留，以及是否获得了稳定收益。
