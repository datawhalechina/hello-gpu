...
这一节先回看你已经做过的事，再引入 Agent。

假设我们要优化 `output = x + y`。数学公式很短，工作却没有随着公式一起结束：你需要确认张量形状和数据类型，读懂当前实现，改变一次数据划分，然后编译、对答案、计时。如果新版本变慢了，还要回到代码里寻找原因。
...
这一节把最小循环放回完整的算子实验，说明每一步要解决什么问题。
...
这一节划清实践范围，避免把教学示例误解为通用优化系统。

本篇处理的是一个给定任务中的 Triton 算子。任务规格定义输入与输出，参考实现负责正确性，模型在这组约束内修改候选。CUDA 或 HIP C++ 代码可以作为理解和转换的输入，但当前评测器不直接编译这些语言；转换后的 Triton 版本仍需重新验证。
...
**实验状态**：本篇当前以实现讲解和实验方法为主。现有依赖锁仍包含 ROCm 7.12 / `gfx1151`，完整 Agent 闭环在 Radeon RX 9070 XT + ROCm 7.13 + 原生 Ubuntu 24.04 上的验证仍待补齐。具体运行入口与前提集中放在 [第 17 章](../chapter17/index.md)，本章不引用旧设备数字作为主线性能结论。

## 14.6 练习：判断一份报告是否可信

这一节通过短练习检查你是否能分清提议、观察和结论。
