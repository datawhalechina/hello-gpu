...
设计工具之前，我们先约定哪些内容可以修改，哪些实验条件必须保持一致。
...
拿到候选源码后，我们先用 `compile_kernel` 检查它能否正确执行。
...
这里的 `profile_kernel` 根据任务工作量、计时结果和设备校准值进行估算，不采集寄存器数量、占用率或访存硬件计数器。我们用它选择下一步实验方向，再通过计时检验假设。
...
候选和当前版本都测过了，接受工具仍需要把它们放在一起重新比较。
...
工具返回错误或拒绝结果时，我们可以根据失败发生的阶段决定下一步检查什么。
...
在 [`tools.py`](https://github.com/datawhalechina/hello-gpu/blob/main/code/part3-agent/kernel_optimize/tools.py) 中，可以沿着工具注册找到这些接口，再进入 [`chapter14/`](https://github.com/datawhalechina/hello-gpu/tree/main/code/part3-agent/chapter14) 阅读评测后端。读完接口后，你可以在 [第 17 章](../chapter17/index.md) 中运行向量加法示例，观察这些结果怎样进入下一轮优化。
