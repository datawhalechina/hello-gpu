...
::: warning 当前实验状态
本章目标环境为 Radeon RX 9070 XT + ROCm 7.13 + 原生 Ubuntu 24.04，但 Part 3 的依赖锁仍保留 ROCm 7.12 / `gfx1151`，当前版本的完整 Agent 实验尚未在主线环境补齐。下列命令按现有入口整理，作为环境迁移完成后的运行步骤；目前不要直接用旧锁文件为 9070 XT 安装依赖。实测时间、接受次数和整体加速比统一为 **🚧 待补实验**。
:::
...
当前入口会在新工作区保留 `baseline.py`，并将它复制为初始 `best.py`。后续接受的候选更新 `best.py`。运行不同任务时使用新工作区；不要把已有工作区参数当作完整的断点续跑功能。
...
密钥留在机器配置中即可，不需要写进教程源码，也不需要提供任何 Git 提交号。环境迁移与验证完成后，在 GPU 机器的仓库根目录执行：
...
这是运行 Agent 并生成图的入口。`--skip-pytest` 跳过的是开发用测试套件，候选的正确性检查仍由评测器执行。只需要运行搜索时，可将最后一行换为 `bash chapter15/run_vector_add.sh`，但这个较短入口不会自动生成可视化图和 `run_summary.json`。

这里尚未给出“预期输出”，因为当前主线实验没有完成。运行后应以终端打印的实际工作区为准，不要寻找教程里某个固定日期的历史目录。

## 17.3 先检查产物，再读性能数字

这一节按文件职责检查一次运行，避免被“程序已退出”误导。

@fig-agent-experiment-artifacts 展示了从任务到报告的信息关系。自动生成的文件能帮助我们检查一部分过程，但不代表完整的复现实验档案已经自动齐备。
...
flowchart TB
    ...
    C --> R[/报告：结果、证据与限制/]
    E[/另存候选源码、调用记录与原始样本/] -.-> R
```

运行状态、搜索过程与最终复测分别支持报告中的不同结论；完整证据还需要保存对应源码和原始记录。
:::
...
最后检查 `best.py` 与 `baseline.py`，确认当前版本究竟改了什么。`trajectory.jsonl` 不包含所有候选的完整源码、完整提示词或原始采样；只调用编译工具就失败的尝试，也不一定出现在其中。当前入口的终端展示还会截断工具观察，因此保存终端输出也不能自动补齐所有原始信息。

如果要把一次运行写成可复核的公开成绩，除了这些摘要，还需要另行保存重要候选源码、任务与环境、工具调用、正确性输出和原始计时样本。缺哪项就说明哪项，不从图或最终报告倒推出一份“原始日志”。
...
搜索结束后，应固定最终 `best.py`，重新与原始 `baseline.py` 进行同口径配对测量。现有评测器提供 `compare.py` 入口；在已经准备好的环境中，将路径替换为本次实际工作区：
...
这里的尖括号是待替换路径，不是可以原样执行的参数。比较输出应保存在本次实验目录中，并与任务、环境和源码一起核对。当前主线的这一步仍为 **🚧 待补实验**。
...
报告模板见 [向量加法实验报告](./optimization-report.md)。模板中的待测项应由本次证据填写；没有原始记录时保持待补，不填一个“符合预期”的数字。
...
- [向量加法实验报告模板](./optimization-report.md)：按证据填写结果与限制。
