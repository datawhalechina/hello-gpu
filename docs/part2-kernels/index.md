...
当前阶段只完整发布[第 7 章 Element-Wise](./chapter7/index.md)。第 8–12 章是后续 phase 的明确大纲；它们只说明学习顺序和预定实验边界，不代表代码、性能数字或结论已经完成。
...
| 2 | [第 8 章 Reduction：归约算子](./chapter8/index.md) | 多个输入共同生成较少输出时，怎样做跨线程协作 | 后续 phase 大纲 |
| 3 | [第 9 章 Softmax（Normalization）](./chapter9/index.md) | 怎样把数值稳定的归约与逐元素计算组合起来 | 后续 phase 大纲 |
| 4 | [第 10 章 GEMM-Like：矩阵乘类算子](./chapter10/index.md) | 怎样用 tile、数据复用和寄存器累加提高计算强度 | 后续 phase 大纲 |
| 5 | [第 11 章 Attention/Fusion](./chapter11/index.md) | 怎样在线计算并减少中间结果的全局写回 | 教学版前向，后续 phase 大纲 |
| 6 | [第 12 章 综合实战：Fused RMSNorm](./chapter12/index.md) | 怎样综合逐元素、归约与融合，独立完成完整优化闭环 | 后续 phase 大纲 |
...
第 7 章的发布表格只读取 `chapter7/evidence/` 中的 curated evidence；实验环境、参数、源码身份、负结果和复跑流程记录在 `chapter7/EXPERIMENT.md`。后续章节在各自实现 phase 完成前，不写入未经实验支持的性能数字。
