...
脚本先跑 `33×257` 的边界输入，再跑主形状，并打印每个实现的 `RESULT`。这是一轮复跑；正文三进程结果的采集参数和来源另见[实验记录](https://github.com/datawhalechina/hello-gpu/blob/dev/code/part2-kernels/chapter13/EXPERIMENT.md)。需要采集 trace 时，在同一个环境中运行下面的独立入口，源码提交号由脚本自动记录：

```bash
PROFILE_WARMUP=0 PROFILE_REPEAT=5 bash chapter13/profile_all.sh
```
