...
with (repo / 'code/part2-kernels/chapter13/evidence/summary.csv').open() as f:
...
fig, axes = plt.subplots(1, 2, figsize=(13,5.8), squeeze=False)
...
for panel, ax in enumerate(axes[0]):
    ...
    for i,r in enumerate(selected):
        ...
        ax.text(hi+(1.9, 0.075)[panel]*.025,i,f'{med:.5f}',va='center',fontsize=10,color='#0f172a')
    ax.set_yticks(range(len(selected)),[r['implementation'] for r in selected])
    ax.invert_yaxis(); ax.set_xlim(0,(1.9, 0.075)[panel]); ax.grid(axis='x',color='#e2e8f0');ax.set_axisbelow(True)
...
fig.text(.16,.945,'Chapter 13 · RMSNorm',fontsize=19,weight='bold',color='#0f172a')
fig.text(.16,.895,'RX 9070 XT · ROCm 7.13 · Ubuntu 24.04 · 1024×4096 FP32',fontsize=11,color='#475569')
...
fig.text(.16,.05,'All versions already use one kernel. Inputs differ between HIP and Triton.',fontsize=10,color='#64748b')
fig.savefig(image_dir/'rmsnorm-performance.svg',facecolor='white');plt.close(fig)
