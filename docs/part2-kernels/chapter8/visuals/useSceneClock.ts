 ...
 * 因此「跳到某一步的稳定状态」= seek(stepStart + TRANSITION_MS)。
 */
export const TRANSITION_MS = 700

/**
 * 场景时钟：单一 time（ms）驱动一切。
 * - 播放：rAF 按真实时间推进，到末尾循环。
...
export function useSceneClock(durations: Ref<number[]>) {
  ...
  function step(now: number) {
    ...
    if (tween) {
      ...
      time.value += dt
      if (time.value >= total.value) time.value = 0 // 循环播放
    } else {
      stopLoop()
      return
    }
  ...
  function goToStep(i: number) {
    ...
    const settled = starts.value[idx] + Math.min(TRANSITION_MS, durations.value[idx])
    glideTo(settled)
  }
