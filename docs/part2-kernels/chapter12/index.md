...
2. 编译 HIP；
3. 用 `1×1`、`3×13`、`33×257` 检查最小输入、尾部和跨 wave/block 边界；
4. 用 `1024×4096` 跑教学主形状；
5. HIP/Triton 每个版本都打印 `RESULT` 行。

当前 Triton 教学 kernel 要求 `cols <= 65536`；HIP 入口使用 `int` 索引，并在分配和 launch 前检查 `rows × cols`、设备最大线程数与 Grid X。更大的行宽或张量需要改成分块/多阶段实现，不能只放大 CLI 参数。
