...
PROFILE_WARMUP="${PROFILE_WARMUP:-0}"; PROFILE_REPEAT="${PROFILE_REPEAT:-5}"; PROFILE_DIR="${SCRIPT_DIR}/profiles"; BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hello-gpu-ch11-profile.XXXXXX")"
...
printf 'source_commit=%s\nseq=%s\ndim=%s\nblock=%s\nseed=%s\n' "${SOURCE_COMMIT}" "${SEQ}" "${DIM}" "${BLOCK}" "${SEED}" > "${PROFILE_DIR}/profile_config.env"
